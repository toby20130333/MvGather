...
//    int viewWith = this->width()-this->verticalScrollBar()->width();
    int columnMax=0;
    if(viewWith >500){
        columnMax=qFloor(viewWith/200)-1;
    }else
    {
        columnMax=2;
    }
    ...
    if(viewWith<500)
    {
        int heightMax=(qFloor(areaWidgetLayout->count()/(columnMax))+1+0.3)*340;
        areaWidget->resize(500,heightMax);
    }else
    {
        int heightMax=(qFloor(areaWidgetLayout->count()/(columnMax+1))+1+0.3)*340;
